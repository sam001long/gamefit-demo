...
// ------------- 全域狀態 -------------
let detector = null;
let currentFacing = "user"; // 手勢 demo 預設用前鏡頭比較好
let running = true;

// finger chains: [wrist, ... joints ... tip]
const fingerChains = [
  [0, 1, 2, 3, 4],   // thumb
  [0, 5, 6, 7, 8],   // index
  [0, 9, 10, 11, 12],// middle
...
// 每根手指的關鍵 index（MediaPipe Hands 的標準）
const fingerDefs = {
  index:  { tip: 8,  pip: 6 },
...
function getFingerStates(hand) {
  ...
  for (const name of Object.keys(fingerDefs)) {
    ...
    const extended = dTip > dPip * 1.2;
    states[name] = { extended, dTip, dPip };
  }
  return states;
}

// 利用手指狀態推估 Rock / Paper / Scissors
...
async function detectionLoop() {
  ...
  let hands = [];
  try {
    hands = await detector.estimateHands(video, { flipHorizontal: currentFacing === "user" });
  } catch (e) {
...
async function main() {
  ...
  statusEl.textContent = "請把一隻手放到畫面中央，試著比出石頭 / 剪刀 / 布。";
